...
def visualize_atom_retention(viz_dir: str,
                             smiles: str,
                             num_atoms: int,
                             retention_weights: torch.FloatTensor):
    """
    Saves figures of retention maps between atoms. Note: works on a single molecule, not in batch
    :param viz_dir: Directory in which to save retention map figures.
    :param smiles: Smiles string for molecule.
    :param num_atoms: The number of atoms in this molecule.
    :param retention_weights: A num_atoms x num_atoms PyTorch FloatTensor containing retention weights.
    """
    if type(smiles) == str:
        mol_name = smiles
    ...
    # retention_weights = (retention_weights[4] + retention_weights[5]) / 2

    for a in range(num_atoms):
        a_weights = retention_weights[a].cpu().data.numpy()
...
# Todo
def visualize_bond_retention(viz_dir: str,
                             mol_graph: None,
                             depth: int,
                             retention_weights: torch.FloatTensor):
    """
    Saves figures of retention maps between bonds.
    :param viz_dir: Directory in which to save retention map figures.
    :param mol_graph: BatchMolGraph containing a batch of molecular graphs.
    :param retention_weights: A num_bonds x num_bonds PyTorch FloatTensor containing retention weights.
    ...
    for i in range(mol_graph.n_mols):
        ...
        for b in range(b_start, b_start + b_size):
            ...
            b_weights = retention_weights[i][b].cpu().data  # 1 x num_bonds
