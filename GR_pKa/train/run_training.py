...
def run_training(args: TrainArgs,
    ...
    # Initialize scaler and scale training targets by subtracting mean and dividing standard deviation (regression os.save_dirnly)
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for epoch in trange(args.epochs):
            ...
            rmse.append(avg_val_score)
            if args.minimize_score and avg_val_score < best_score or \
                    not args.minimize_score and avg_val_score > best_score:
                best_score, best_epoch = avg_val_score, epoch
                save_checkpoint(os.path.join(save_dir, f'model_{epoch}.pt'),
                                model, scaler, features_scaler, args)
                best=epoch
        ...
        model = load_checkpoint(os.path.join(
            save_dir, f'model_{best}.pt'), device=args.device, logger=logger)
