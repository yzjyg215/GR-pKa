...
class MPNEncoder(nn.Module):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int):
        ...
        self.bond_retention = args.bond_retention
        self.bond_fast_retention = args.bond_fast_retention
        ...
        if self.bond_retention:
            self.bond_retention_block = MultiBondRetention(args)
            # self.bond_residual = SublayerConnection(dropout=self.dropout)

        if self.bond_fast_retention:
            self.bond_retention_block = MultiBondFastRetention(args)
            # self.bond_residual = SublayerConnection(dropout=self.dropout)

        if self.atom_retention:
            self.atom_retention_block = MultiAtomRetention(args)
    ...
    def forward(self,
        ...
        for depth in range(self.depth - 1):
            ...
            if self.atom_messages:
                ...
                if self.bond_retention or self.bond_fast_retention:
                    message = self.bond_retention_block(message, b_scope)
                    # message: batch_num_bonds x hidden_size
                    # message = self.bond_residual(message, ret_message)
        ...
        for i, (a_start, a_size) in enumerate(a_scope):  # a_scope: (1, num_atoms)
            if a_size == 0:
                ...
                if self.atom_retention:
                    mol_vec, ret_a_w = self.atom_retention_block(
